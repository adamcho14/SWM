...
t_d1 = 150  # delay - before distractor presentation
t_dis = t_p  # delay - presentation of the distractor
t_d2 = 1100  # delay - after distractor presentation
t_s = 50  # response
t_e = 50  # post-trial

t_d = t_d1 + t_dis + t_d2  #delay - total

ms = t_0 + t_p + t_d1 + t_dis + t_d2 + t_s + t_e  # simulation time in ms - pre-presentation, presentation, delay, saccade, post-delay

timestep = 0.02 # simulation time step

time = int(float(ms) / float(timestep)) # number of simulation steps
print time
presented_cue = 180  # presented cue
distractor_cue = 91  # distractor cue angle
...
for t in range(time):
    ...
    if t == float(t_0) / float(timestep) or t == float(t_0 + t_p) / float(timestep) or t == float(
                            t_0 + t_p + t_d1 + t_dis + t_d2) / float(timestep):
    ...
    for i in range(n_e + n_i):
        ...
        if float(t_0 + t_p + t_d1) / float(timestep) <= t < float(
                                                        t_0 + t_p + t_d1 + t_dis) / float(timestep):
            if i < n_e:
                test_distractor = I_transient_current(i_transient, distractor_cue, preferred_cue(i, n_e))
                inp[i][t] += test_distractor

        # post-delay period
        if float(t_0 + t_p + t_d1 + t_dis+ t_d2) / float(timestep) <= t < float(t_0 + t_p + t_d1 + t_dis + t_d2 + t_s) / float(timestep):
            inp[i][t] += i_response
